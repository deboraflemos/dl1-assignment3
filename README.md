...
---
### Answer to the thought exercise
For this assignment, we predicted the value of each vault through a linear model.

We can check the estimated weights (values) of gold and silver coins by looking at the following variables:
```
weights = model.weight.data
biases = model.bias.data
```
However, a linear model has a bias component that does not exist in our theoretical problem, so in order to get a more precise estimation of the value of a gold coin, we should force the bias to be zero.
